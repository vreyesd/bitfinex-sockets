...
class PrivateSocket {
  ...
  constructor(
    ...
    this.setOnCloseCallback = this.setOnCloseCallback.bind(this);
  ...
  onClose() {
    ...
    if (this.mustReconnect) {
      print("reconnecting...");
      if (this.wss) this.wss.removeAllListeners();

      this.reconnectTimer = setTimeout(() => {
        if (!this.mustReconnect) return;
  ...
  /**
   * Set function triggered when socket close
  ...
  newOrder({
    ...
    // using strict checks ( !== undefined ) to prevent breaking zero valuations like missing flags parameters if flag=0
    if (price !== undefined) order.price = price;
    if (lev !== undefined) order.lev = lev;
    if (price_trailing !== undefined) order.price_trailing = price_trailing;
    if (price_aux_limit !== undefined) order.price_aux_limit = price_aux_limit;
    if (price_oco_stop !== undefined) order.price_oco_stop = price_oco_stop;
    if (flags !== undefined) order.flags = flags;
    if (tif !== undefined) order.tif = tif;
  ...
  updateOrder({
    ...
    if (cid !== undefined) order.cid = cid;
    if (cid_date !== undefined) order.cid_date = cid_date;
    if (gid !== undefined) order.gid = gid;
    if (price !== undefined) order.price = price;
    if (amount !== undefined) order.amount = amount;
    if (lev !== undefined) order.lev = lev;
    if (delta !== undefined) order.delta = delta;
    if (price_aux_limit !== undefined) order.price_aux_limit = price_aux_limit;
    if (price_trailing !== undefined) order.price_trailing = price_trailing;
    if (flags !== undefined) order.flags = flags;
    if (tif !== undefined) order.tif = tif;
   ...
   * @param {String} cid_date Client Order ID Date
   */
  cancelOrder({ id, cid, cid_date }) {
    if (!id && !(cid && cid_date)) throw new Error("id is required ");

    const order = {};

    if (id !== undefined) order.id = id;
    if (cid !== undefined) order.cid = cid;
    if (cid_date !== undefined) order.cid_date = cid_date;
  ...
  cancelOrderMulti(orders) {
    if (!orders) throw new Error("orders payload is required");
   ...
   * @param {Number} flags See https://docs.bitfinex.com/v2/docs/flag-values
   */
  newOffer({ type, symbol, amount, rate, period, flags }) {
    if (
      type === undefined ||
      symbol === undefined ||
      amount === undefined ||
      ...
      throw new Error(
        "type, symbol, amount, rate, period and flags are required ",
      );
