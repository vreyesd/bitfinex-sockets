...
class PublicSocket {
  ...
  get acceptSubscribe() {
    const length =
      this.channelToConnect.length +
      this.channelConnecting +
      this.channelConnected.size;
  ...
  init() {
    ...
    // Use Node.js event emitter syntax which is more performant than property assignment
  ...
  onMessage(message) {
    let data;
    try {
      // Optimize: Parse once instead of multiple times
    ...
    // Fast path for data events (array format), which represent ~99% of ws stream volume
    if (!Array.isArray(data)) {
      if (data.event === "subscribed") {
        this.channelConnecting = Math.max(0, this.channelConnecting - 1);

        let connected = { ...data };
        delete connected.event;

        // Use Map for O(1) performance lookup and updates instead of O(N) Array looping
        this.channelConnected.set(data.chanId, connected);
      } else if (data.event === "unsubscribed" && data.status === "OK") {
        this.channelConnected.delete(data.chanId);
      } else if (data.event === "error") {
        // If a subscription logic fails, free up channelConnecting allocation
        this.channelConnecting = Math.max(0, this.channelConnecting - 1);
        print(`Bitfinex Error: ${data.msg || JSON.stringify(data)}`);
      }
    }
  ...
  onClose() {
    ...
    if (this.mustReconnect) {
      ...
      const toConnect = Array.from(this.channelConnected.values());
      this.channelConnected.clear();
      this.channelConnecting = 0;
